...
- `--page-size`: Output canvas preset. Use `A4`, `A5`, `B5`, or `LETTER` for a fixed paper size, or `ORIGINAL` to keep each page at its processed image size.
...
- `--deskew` and `--no-deskew`: Enables or disables OpenCV-based skew correction for slightly rotated text lines. Deskew is enabled by default.
...
- `--global-scale` and `--no-global-scale`: When trimming margins, keeps one shared scale factor across all pages so the visible content stays at a consistent size. Global scale is enabled by default.
